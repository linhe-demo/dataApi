...
type MouldFestival struct {
	...
	FestivalName string    `gorm:"index;column:festival_name;type:varchar(255);not null"` // 节日名称
	Type         int16     `gorm:"column:type;type:smallint(6)"`                          // 类型 1=节日名，2=中文祝福语，3=英文祝福语言，4=按钮文字，5=节日祝福语, 6=封面图
...
func GetFestivalData(mouldId int64, festivalName string) (out MouldFestival, err error) {
	tx := app.MysqlClient.Where("MouldID = ? AND FestivalName = ?", int(mouldId), festivalName).Find(&out)
